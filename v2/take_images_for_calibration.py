...
CamR= cv2.VideoCapture(6)   # 0 -> Right Camera
...
while True:
    ...
    retR, cornersR = cv2.findChessboardCorners(grayR,chess_board_corners,None)  # Define the number of chess corners (here 9 by 6) we are looking for with the right Camera
    retL, cornersL = cv2.findChessboardCorners(grayL,chess_board_corners,None)  # Same with the left camera
    ...
    if (retR == True) & (retL == True):
        ...
        cv2.drawChessboardCorners(grayR,chess_board_corners,corners2R,retR)
        cv2.drawChessboardCorners(grayL,chess_board_corners,corners2L,retL)
